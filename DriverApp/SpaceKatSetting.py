...
##################################################
### tkinter和customtkinter用于创建图形用户界面   ###
### spinbox是自定义的控件                       ###
### PIL用于图像处理                             ###
### os和sys用于操作系统相关的操作                ###
### json用于处理JSON格式的数据                  ###
### serial.tools.list_ports用于列出可用的串口   ###
##################################################
...
print (configFile)
configuration = {}
configuration['TimeStamp'] = time.time() #用于存储配置文件的时间戳
configuration['Basic'] = {}  #用于存储基础设置
configuration['Speed'] = {}  #用于存储速度设置
...
comPort = "COM7"
comPorts = []
comPortList = serial.tools.list_ports.comports() #列出可用的串口
for com in comPortList:
    #if com[0] != comPort:
    comPorts = comPorts + [com[0]] #将可用的串口添加到comPorts列表中 com[0]是串口名称
